...
valid_situations = ['morning', 'morning-simulation', 'daytime', 'evening', 'nighttime', 'nighttime-simulation']
...
if level is None or 'eg' in level:
    ensure_state(sunprot_eastface_is_active, 'cover.f1_cov_wohnzimmer_sitzfenster_cover')
    # Für den Sonnenschutz werden entweder die Markise oder die Raffstore benötigt, aber nicht beides gleichzeitig.
    ensure_state(sunprot_eastface_is_active and not use_terrasse_markise, 'cover.f1_cov_wohnzimmer_terrassentur_cover')
    ensure_state(sunprot_eastface_is_active and use_terrasse_markise, 'cover.f1_cov_terrasse_markise_cover')
    ensure_state(sunprot_wiga_is_active, 'cover.f1_cov_wintergarten_markise_cover')
    ensure_state(sunprot_southface_is_active, 'cover.f1_cov_kuche_terrassentur_cover')
    ensure_state(sunprot_westface_is_active, 'cover.f1_cov_windfang_cover')
    ensure_state(sunprot_westface_is_active, 'cover.f1_cov_kuche_fenster_cover')

if level is None or 'og' in level:
    ensure_state(sunprot_eastface_is_active, 'cover.f2_cov_florian_cover')
    ensure_state(sunprot_eastface_is_active, 'cover.f2_cov_jonathan_links_cover')
    ensure_state(sunprot_southface_is_active, 'cover.f2_cov_jonathan_rechts_cover')
    ensure_state(sunprot_westface_is_active, 'cover.f2_cov_schlafzimmer_cover')
    ensure_state(sunprot_westface_is_active, 'cover.f2_cov_bad_cover')
    ensure_state(sunprot_westface_is_active, 'cover.f2_cov_flur_cover')

if level is None or 'dg' in level:
    ensure_state(sunprot_eastface_is_active, 'cover.f3_cov_studio_links_cover')
    ensure_state(sunprot_eastface_is_active, 'cover.f3_cov_studio_rechts_cover')
    # Die innenliegende Verschattung wird nicht für den Sonnenschutz benutzt, daher ist `sunprot_is_active` immer False.
    ensure_state(False, 'cover.f3_cov_dachfenster_gross_cover')
    ensure_state(sunprot_southface_is_active or sunprot_westface_is_active, 'cover.f3_cov_dachfenster_gross_aussen_cover')
    # # Im Bad die Beschattung herunterfahren, sobald irgendein Sonnenschutz aktiv ist, da es sonst dort sehr warm wird.
    ensure_state(sunprot_southface_is_active or sunprot_westface_is_active, 'cover.f3_cov_bad_cover')
